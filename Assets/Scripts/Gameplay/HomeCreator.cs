...
public class HomeCreator : MonoBehaviour
{
    ...
    //private RectTransform _homePosition;
    ...
    private void Awake()
    {
        ...
        //_homePosition = GetComponent<RectTransform>();
    }

    public void UpdateVisual(HomeInfo homeInfo)
    {
        capitalHomePrefab.GetComponent<Image>().sprite = homeInfo.CapitalHomePrefab;
        home1Prefab.GetComponent<Image>().sprite = homeInfo.Home1Prefab;
        home2Prefab.GetComponent<Image>().sprite = homeInfo.Home2Prefab;

        if (_capitalBlock != null) _capitalBlock.sprite = homeInfo.CapitalHomePrefab;
        foreach (var image in _home1Blocks)
        {
            image.sprite = homeInfo.Home1Prefab;
        }
        foreach (var image in _home2Blocks)
        {
            image.sprite = homeInfo.Home2Prefab;
        }
    }
